...
<dt>Directories</dt>
<dd>You have the option of using either relative or absolute paths when appending splits or creating the plane. Relative paths are shorter but will cause errors when you change working directories (with <samp>:cd [dir]</samp>); you will have to maintain a consistent working directory. Absolute paths are harder to type but changing working directories won't be a problem. Using the home prefix ('~') might be a good idea when typing absolute paths. If you find yourself needing to change a lot of file names at once, you can `hotkey``W`riting the plane to file and then edit that file.</dd>
