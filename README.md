...
- The **recommended setup** for textabyss is terminal emulator, since <samp>ttymouse</samp> supports intercepting raw mousecodes rather than working off of cursor position. (This, for example, allows you to drag from anywhere on the screen rather than only lines containing text, panning in map mode, hot corners, mouse acceleration, and so on.) The recommended version is > 7.4.131 since patch 131 fixes a scrollbind bug which otherwise would require a scripting workaround. For these reasons, **[Cygwin](http://www.cygwin.com/)**, running the (bundled) [mintty](https://code.google.com/p/mintty/) terminal emulator is recommended for running Vim on Windows over gVim (which is in turn recommended over running Vim from the Windows command prompt). Cygwin also updates Vim far more frequently. If you have a slower computer, **cleaning up autocommands** (as recommended in the dynamically generated "Warnings and Suggestions" section of the in-script help) will make a difference in panning smoothness.
